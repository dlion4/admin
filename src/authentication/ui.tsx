      ...
      >
        ...
        >
          {icon && (
            ...
            >
              {typeof icon === "string" ? <i className={`bi bi-${icon}`} /> : icon}
            </span>
          )}
          <div className="min-w-0 flex-1">
    ...
    <div className="pm-tabs">
      {tabs.map((t) => (
        <button key={t.id} className={`pm-tab ${active === t.id ? "active" : ""}`} onClick={() => onChange(t.id)}>
          {t.icon && <span className="mr-1.5 flex-none">{t.icon}</span>}
          {t.label}
          {t.badge !== undefined && (
            <span
    ...
    <div className="mx-auto w-full max-w-[16rem]">
      <div className="mb-2 flex items-center justify-between">
        ...
        <button className="pm-btn pm-btn-ghost pm-btn-sm" onClick={shuffle} type="button">
          <i className="bi bi-shuffle" /> {scrambled ? "Re-scramble" : "Scramble"}
        </button>
      </div>
