...
import { GrantedStage, IdentityStage, PasskeyGate, PinGate, SessionPinGate, TotpGate } from "../Gates";
import { Badge, Drawer, Modal, Popover, StatDot, Tabs, fmtMs, useToast } from "../ui";
import { ToastHost } from "../../components/ui";
import "../authentication.css";

type StageId = "identity" | "role" | "pin" | "passkey" | "totp" | "session" | "granted";

const STEPS: { id: StageId; label: string; hint: string; icon: ReactNode; tone: string }[] = [
  { id: "identity", label: "Credentials", hint: "Email + password", icon: <i className="bi bi-envelope-fill" />, tone: "var(--bs-secondary)" },
  { id: "role", label: "Role context", hint: "Declare your clearance", icon: <i className="bi bi-person-badge" />, tone: "var(--bs-primary)" },
  { id: "pin", label: "Admin PIN", hint: "6 digits · 3 attempts", icon: <i className="bi bi-123" />, tone: "var(--bs-primary)" },
  { id: "passkey", label: "Passkey", hint: "FIDO2 / WebAuthn", icon: <i className="bi bi-key-fill" />, tone: "var(--bs-purple)" },
  { id: "totp", label: "TOTP", hint: "30-second rotation", icon: <i className="bi bi-stopwatch-fill" />, tone: "var(--bs-teal)" },
  { id: "session", label: "Session PIN", hint: "Super admin issued", icon: <i className="bi bi-shield-check" />, tone: "var(--bs-danger)" },
  { id: "granted", label: "Access granted", hint: "Dashboard unlocked", icon: <i className="bi bi-check-circle-fill" />, tone: "var(--bs-success)" },
];
...
export function AuthenticationPage({ signal, onNavigate }: AuthenticationPageProps) {
  ...
  return (
    <div className="auth-page relative min-h-screen" data-auth="">
      ...
      <div className="relative z-10 mx-auto max-w-[95rem] px-3 py-4 sm:px-5 lg:px-7">
        ...
        <header className="pm-card mb-4 px-4 py-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="ml-auto flex flex-wrap items-center gap-2">
              ...
              <Popover label={<><i className="bi bi-lightbulb" /> <span className="hidden sm:inline">Demo keys</span></>} tone="var(--bs-info)">
                ...
                <div className="space-y-1.5">
                  {[
                    ...
                    ["PIN", DEMO.pin],
                    ["Passkey", "Pick YubiKey → Authenticate"],
                    ["GTOTP", "Read the simulated authenticator"],
                    ["Session PIN", "Request it from the super admin"],
                  ].map(([k, v]) => (
              ...
              <button className="pm-btn pm-btn-dark pm-btn-sm" onClick={() => setConsoleOpen(true)}>
                <i className="bi bi-pc-display-horizontal" /> <span className="hidden sm:inline">Security console</span>
              ...
              <button className="pm-btn pm-btn-danger pm-btn-sm" onClick={() => setLockdownOpen(true)}>
                <i className="bi bi-exclamation-triangle-fill" /> <span className="hidden sm:inline">Lockdown</span>
              </button>
            </div>
          </div>

          {/* gate flow strip */}
          <div className="auth-gates-strip">
            {STEPS.map((s, i) => {
              ...
              return (
                ...
                >
                  <span className="flex-none">{st === "done" ? <i className="bi bi-check-lg" /> : s.icon}</span>
        ...
        {/* ================= Hero ================= */}
        {stage !== "granted" && (
          <section className="auth-hero mb-4">
            <div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {GATE_SPECS.map((g) => {
                ...
                return (
                  ...
                  >
                    <span className="text-base flex-none">{isDone ? <i className="bi bi-check-circle-fill" style={{color:"var(--bs-success)"}} /> : g.icon}</span>
                    <div>
          ...
          <main className="min-w-0">
            ...
            {stage === "role" && (
              <RoleStage
                ...
                onContinue={() => {
                  if (!roleId) {
                    toast({ tone: "warning", icon: <i className="bi bi-person-badge" />, title: "Pick a role first", body: "The gates adapt to the clearance you declare." });
                    return;
                  }
      ...
      >
        <Tabs
          ...
          tabs={[
            { id: "audit", label: "Audit trail", icon: <i className="bi bi-journal-text" />, badge: audit.length },
            { id: "sessions", label: "Sessions", icon: <i className="bi bi-pc-display-horizontal" /> },
            { id: "policy", label: "Policy", icon: <i className="bi bi-rulers" /> },
            { id: "threat", label: "Threats", icon: <i className="bi bi-exclamation-diamond" /> },
          ]}
        />
        <div className="pt-3">
          {consoleTab === "audit" && (
      ...
      >
        <Tabs
          ...
          onChange={setHelpTab}
          tabs={[
            { id: "gates", label: "The 4 gates", icon: <i className="bi bi-door-open" /> },
            { id: "flow", label: "Flow", icon: <i className="bi bi-diagram-3" /> },
            { id: "session", label: "Session management", icon: <i className="bi bi-clock-history" /> },
            { id: "tiers", label: "Role tiers", icon: <i className="bi bi-bank" /> },
          ]}
        />
        <div className="pt-3">
          {helpTab === "gates" && (
...
// Wrapper component that provides ToastHost
export function AuthenticationPageWrapper(props: AuthenticationPageProps) {
  return (
    <ToastHost>
