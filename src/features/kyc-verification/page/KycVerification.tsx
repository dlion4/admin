...
export function KycVerification({ signal, onNavigate }: { signal: { action: string; n: number }; onNavigate: (id: string) => void }) {
  ...
  return <>
    ...
    <BulkDecisionModal open={bulkOpen} count={selected.length} onClose={() => setBulkOpen(false)} onDone={(action) => { const patch: Partial<KycCase> = action === "approve" ? { state: "Approved" } : action === "reject" ? { state: "Rejected" } : action === "info" ? { state: "More info" } : action === "assign" ? { reviewer: "Jeckonia Kwasa", state: "In review" } : { state: "In review" }; setCases((list) => list.map((c) => selected.includes(c.id) ? { ...c, ...patch } : c)); push({ kind: "success", title: `${action} applied to ${selected.length} cases` }); setSelected([]); }} />
