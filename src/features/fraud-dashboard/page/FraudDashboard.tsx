...
export function FraudDashboard({ signal, onNavigate }: { signal: { action: string; n: number }; onNavigate: (id: string) => void }) {
 ...
 return (
  <>
   ...
   {newBlock && <BlacklistModal onClose={() => setNewBlock(false)} onAdd={(value) => { const e: BlacklistEntry = { id: `BLE-${String(502 + blacklist.length).padStart(4, "0")}`, type: "Device fingerprints", value: value || "New manual entity", reason: "Investigator-managed block", added: "Just now", addedBy: "Joseph Mwangi", hits: 0 }; setBlacklist(x => [e, ...x]); setNewBlock(false); push({ kind: "success", title: "Blacklist entry created", body: "The entity is now blocked across applicable channels." }); }} />}
