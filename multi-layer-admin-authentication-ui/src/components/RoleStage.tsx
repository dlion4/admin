...
const PERM_ICON: Record<PermState, string> = { full: "✅", config: "⚙️", none: "❌" };
    ...
    >
      ...
      <div className="relative mt-2.5 flex items-center justify-between gap-2">
        <div className="flex items-center gap-1 text-[0.65rem]">
          <span className="pm-chip !px-1.5 !py-0.5">✅ {s.full}</span>
          <span className="pm-chip !px-1.5 !py-0.5">⚙️ {s.config}</span>
          <span className="pm-chip !px-1.5 !py-0.5">❌ {s.none}</span>
        </div>
        <button
    ...
    <div className="pm-fade-up" key={role.id}>
      <Tabs
        ...
        tabs={[
          ...
          { id: "perms", label: "Permissions", icon: "🔓", badge: `${s.full}/${s.total}` },
          { id: "guard", label: "Guardrails", icon: "🚧" },
          { id: "team", label: "Seats", icon: "👤", badge: role.seats.active },
        ]}
      />
      <div className="pt-3">
        {tab === "overview" && (
      ...
      footer={
        <>
          <span className="pm-faint mr-auto text-[0.7rem]">
            ✅ granted · ⚙️ configurable · ❌ denied — highlighted column = selected role
          </span>
